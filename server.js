...
app.post('/create-video', upload.array('images'), async (req, res) => {
  ...
  const imageList = files.map((f) => f.path).join('|');
  const outputPath = `uploads/output_${Date.now()}.mp4`;

  // ✅ Dùng concat ảnh cụ thể
  const cmd = `ffmpeg -y -framerate 1 -pattern_type glob -i "uploads/*.jpg" -c:v libx264 -r 30 -pix_fmt yuv420p ${outputPath}`;
